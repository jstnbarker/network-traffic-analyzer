...
def process_packet(packet, print_all, print_attacks, print_tcp, print_udp, print_icmp):
    ...
    if TCP in packet:
        ...
        if packet[TCP].flags == 'F':  # Check for FIN flag
            ...
            if fin_time is None:  # If this is the first FIN packet
                ...
                if packet.time - fin_time >= 1:  # If it's been at least one second since the last FIN packet
                    if fin_counter > 100:  # If more than 100 FIN packets were received in the last second
                        if print_tcp or print_all or print_attacks:
        ...
        if DNS in packet and packet[DNS].qr == 1:  # Check if the packet is a DNS response
            ip_src = packet[IP].src
        ...
        # Handle fragmented UDP packets
        if packet.haslayer(IP) and int(packet[IP].frag) > 0:
            ip_src = packet[IP].src
            if ip_src not in dns_amplification_state:
                dns_amplification_state[ip_src] = 1
            else:
        ...
        if packet[ICMP].type == 8 and packet[IP].dst.endswith('.255'):  # ICMP echo request with broadcast destination
            smurf_counter += 1
            if smurf_time is None:  # If this is the first potential Smurf packet
                ...
                if packet.time - smurf_time >= 1:  # If it's been at least one second since the last potential Smurf packet
                    if smurf_counter > 1000:  # If more than 3 potential Smurf packets were received in the last second
